/* SmolScheme
 *   Simon Boerwinkle (sboerwinkle@gmail.com)
 *   Based on TINYSCHEME (original credits follow)
 *
 * T I N Y S C H E M E    2 . 4 1
...
#define MARK_RUN_START  64    /* 0000000001000000 */ // Used on unallocated cells
#define MARK_RUN_END   128    /* 0000000010000000 */ // Used on unallocated cells
#define T_SYNTAX      4096    /* 0001000000000000 */
// old: T_IMMUTABLE   8192    /* 0010000000000000 */
...
#define unalloced_next(p)   ((p)->_object._unalloced_range.next)
#define unalloced_length(p) ((p)->_object._unalloced_range.length)
...
INTERFACE INLINE int is_c_ptr(pointer p, int type) { return type(p)==T_C_PTR && strlength(p)==type; }
INTERFACE INLINE void* c_ptr_value(pointer p) { return (void*)strvalue(p); }
...
static pointer get_cell(scheme *sc);
...
static pointer append_vector_elem(scheme *sc, pointer vec, int ielem, pointer a);
...
static int alloc_cellseg(scheme *sc, int n) {
    ...
    for (k = 0; k < n; k++) {
        if (sc->num_cell_segs >= CELL_NSEGMENT) return k;
        newp = (pointer) sc->malloc(CELL_SEGSIZE * sizeof(struct cell));
        if (newp == 0) return k;
        sc->cell_seg[sc->num_cell_segs++] = newp;

        last = newp + CELL_SEGSIZE - 1;
        typeflag(newp) = 0;
        typeflag(last) = 0;
        pointer run_start = newp + 1;
        pointer run_end = last - 1;
        for (p = run_start; p <= run_end; p++) {
             typeflag(p) = MARK;
             // TODO Is this even needed, can we leave them uninitialized?
             cdr(p) = p;
             car(p) = p;
        }

        // Insert the new run as the last option (list of free runs is circular)
        pointer prev_run = car(sc->FREE_CELL);
        car(run_start) = prev_run;
        cdr(run_start) = sc->FREE_CELL;
        car(sc->FREE_CELL) = run_start;
        cdr(prev_run) = run_start;

        typeflag(run_start) |= MARK_RUN_START;
        typeflag(run_end) |= MARK_RUN_END;
        ivalue_unchecked(run_start + 1) = CELL_SEGSIZE - 2;
        ivalue_unchecked(run_end) = CELL_SEGSIZE - 2;
    }
    return n;
}
...
/* get new cell. */
static pointer get_cell(scheme *sc) {
  // Migh consider keeping a simple `cdr(foo)` list for quicker access to a
  // short list of cells being re-alloced for different stuff internally

  if (cdr(sc->FREE_CELL) == sc->FREE_CELL) {
    if(sc->no_memory) {
      sc->sink->references++;
      return sc->sink;
    }
  ...
  pointer donor = cdr(sc->FREE_CELL);
  pointer ret;
  if (typeflag(donor) & MARK_RUN_END) {
    ret = donor;
    pointer next = cdr(donor);
    cdr(sc->FREE_CELL) = next;
    car(next) = sc->FREE_CELL;
  } else {
    int len = ivalue_unchecked(donor+1) - 1;
    ret = donor + len;
    pointer tail = ret - 1;
    if (len > 1) {
      ivalue_unchecked(donor + 1) = len;
      ivalue_unchecked(tail) = len;
    }
    typeflag(tail) |= MARK_RUN_END;
  }
...
static pointer find_consecutive_cells(scheme *sc, int n) {
  pointer run;
  for (run = cdr(sc->FREE_CELL); run != sc->FREE_CELL; run = cdr(run)) {
    if (typeflag(run) & MARK_RUN_END) continue;
    int len = ivalue_unchecked(run+1);
    if (len < n) continue;
    int remainder = len - n;
    pointer ret = run + remainder;
    if (remainder) {
      typeflag(ret - 1) |= MARK_RUN_END;
      if (remainder > 1) {
        ivalue_unchecked(run + 1) = remainder;
        ivalue_unchecked(ret - 1) = remainder;
      }
    } else {
      pointer prev = car(run);
      pointer next = cdr(run);
      cdr(prev) = next;
      car(next) = prev;
    }
    return ret;
...
static pointer get_vector_object(scheme *sc, int len)
{
  // Extra cell is because the first one stores the count,
  // others store cells in their car/cdr
...
{
     pointer vec = mk_vector(sc,462); /* probably should be bigger */
     fill_vector(vec, sc->NIL);
     return vec;
}
...
static pointer oblist_add_by_name(scheme *sc, const char *name)
{
  ...
  sc->NIL->references++;
  x = cons(sc, mk_string(sc, name), sc->NIL);
  typeflag(x) = T_SYMBOL;

  location = hash_fn(name, ivalue_unchecked(sc->oblist));
  append_vector_elem(sc, sc->oblist, location, x);
  x->references++;
  return x;
}

// This is used so often for just checking for existence of a name,
// that we break with convention and don't actually increment references here.
...
static pointer oblist_add_by_name(scheme *sc, const char *name)
{
  ...
  x = cons(sc, mk_string(sc, name), sc->NIL);
  typeflag(x) = T_SYMBOL;
  sc->oblist = cons(sc, x, sc->oblist);
  x->references++;
  return x;
}
static pointer oblist_all_symbols(scheme *sc)
{
  sc->oblist->references++;
  return sc->oblist;
}
...
{ return get_vector_object(sc,len); }
...
// `a` is increm'd, neither `vec` nor the result are. Assumes position was not previously set
INTERFACE static pointer set_vector_elem(pointer vec, int ielem, pointer a) {
     pointer cell = vec+1+ielem/2;
     if(ielem%2==0) {
          return car(cell)=a;
     } else {
          return cdr(cell)=a;
...
// `vec` and the result aren't increm'd, only `a`
static pointer append_vector_elem(scheme *sc, pointer vec, int ielem, pointer a) {
     pointer cell = vec+1+ielem/2;
     pointer *dest;
     if(ielem%2==0) {
     ...
     pointer new_cell = cons(sc, a, *dest);
     return *dest = new_cell;
}
...
INTERFACE pointer mk_c_ptr(scheme *sc, void* value, int type) {
     pointer x = get_cell(sc);
     typeflag(x) = (T_C_PTR | T_ATOM);
     strvalue(x) = (char*) value;
     strlength(x) = type;
     return x;
}
...
#if USE_COLON_HOOK
     // TODO Reference counting not implemented for this block
#error whups, reference counting not implemented here
     if((p=strstr(q,"::"))!=0) {
          *p=0;
...
#ifndef MEM_DEBUG
#warning defining MEM_DEBUG as 1
#define MEM_DEBUG 1
#endif

static void reclaim_run(scheme *sc, pointer x, int len) {
  // Caller is responsible for calling `finalize_cell` if necessary,
  // we're just throwing it back on "the pile".

  // All unallocated cells have typeflag MARK set, and `type(p) == 0`.
  // (note `type` differs from `typeflag`).

  // First cell of a contiguous unallocated run has typeflag MARK_RUN_START set,
  // and its car/cdr participate in a circular doubly-linked list of unallocated runs
  // (`sc->FREE_CELL` being the start/end of said list)

  // The last cell of said unallocated run has typeflag MARK_RUN_END set;
  // if this is the same cell as MARK_RUN_START it is understood to contain only one cell,
  // and otherwise the length of the entire run (end-start+1) is stored in both (end) and (start+1).

  //  Set all type flags to "free"; MARK_RUN_START / _END are set later
  int i;
  for (i = 0; i < len; i++) {
    typeflag(x + i) = MARK;
  }

  //  if its successor is a free cell head:
  //    Determine run length
  int next_len = 0;
  if (typeflag(x + len) & MARK) {
#if MEM_DEBUG
    if (!(typeflag(x + len) & MARK_RUN_START)) {
      fprintf(stderr, "scheme MEM_DEBUG: %p had flags %d, should have included flag %d\n", x+len, typeflag(x + len), MARK_RUN_START);
    }
#endif
    if (typeflag(x + len) & MARK_RUN_END) next_len = 1;
    else next_len = ivalue_unchecked(x + len + 1);
  }

  //  if predecessor is free cell tail:
  //    Determine prev / run length
  int prev_len = 0;
  if (typeflag(x - 1) & MARK) {
#if MEM_DEBUG
    if (!(typeflag(x - 1) & MARK_RUN_END)) {
      fprintf(stderr, "scheme MEM_DEBUG: %p had flags %d, should have included flag %d\n", x-1, typeflag(x - 1), MARK_RUN_END);
    }
#endif
    if (typeflag(x - 1) & MARK_RUN_START) prev_len = 1;
    else prev_len = ivalue_unchecked(x - 1);
  }

  if (next_len) {
    if (prev_len) {
      pointer next = x + len;
      pointer prev = x - prev_len;

      pointer next_run = cdr(next);
      pointer prev_run = car(next);
      car(next_run) = prev_run;
      cdr(prev_run) = next_run;

      typeflag(next) &= ~MARK_RUN_START;
      typeflag(x - 1) &= ~MARK_RUN_END;
      len += prev_len + next_len;
      ivalue_unchecked(prev + 1) = len;
      ivalue_unchecked(prev + len - 1) = len;
    } else {
      pointer next = x + len;
      ...
      typeflag(x) |= MARK_RUN_START;
      typeflag(next) &= ~MARK_RUN_START;
      len += next_len;
      ivalue_unchecked(x + 1) = len;
      ivalue_unchecked(x + len - 1) = len;
    }
  } else if (prev_len) {
    pointer prev = x - prev_len;

    typeflag(x + len - 1) |= MARK_RUN_END;
    typeflag(x - 1) &= ~MARK_RUN_END;
    len += prev_len;
    ivalue_unchecked(prev + 1) = len;
    ivalue_unchecked(prev + len - 1) = len;
  } else {
    pointer next_run = cdr(sc->FREE_CELL);
    cdr(x) = next_run;
    car(x) = sc->FREE_CELL;
    cdr(sc->FREE_CELL) = x;
    car(next_run) = x;
  ...
  // . TODO remove `sc->free_cell` and `fcells`?
  // . TODO `ok_to_freely_gc` referenced `car(sc->sink)`, what's up with that?
  // . TODO replace `last_cell_seg` with `num_cell_segs`, like a functioning primate would do
  // . TODO alloc_seg and cell_seg are now probably the same thing (barring any updates later on)
}

void decrem(scheme *sc, pointer x) {
  pointer parent = NULL;
  while (1) {
    if (--x->references == 0) {
      // Going down
      ...
      if (is_vector(x) && ivalue(x)) {
        pointer child = car(x+1);
        car(x+1) = parent;
        parent = x;
        x = child;
        continue;
    ...
    while (1) {
      ...
      if (is_atom(parent)) {
        if (is_vector(parent)) {
          ...
          // TODO Don't think there's any point in setting this back?
          ...
          int vector_size = (ivalue(parent)+3)/2;
          ix = vector_size;
          ...
          x = parent;
          parent = grandparent;
          reclaim_run(sc, x, vector_size);
        } else {
          clratom(parent);
          pointer child = x; // decrem: This goes away, don't need to point anybody back to the child
          x = parent;
          parent = cdr(x);
          cdr(x) = child; // decrem: This goes away, don't need to point anybody back to the child
          // decrem: x has already visited both children, carefully place it on the scrap heap.
          //   (though ints etc are still an outstanding question)
          // Don't need to finalize this, we know it wasn't atomic (which everything finalizable is)
          reclaim_run(sc, x, 1);
        }
      } else {
        // If car was valid, is cdr always also valid?
        // decrem: This changes a lot; since we don't have to "fix" car(parent),
        //   we also don't have to overwrite cdr(parent) (meaning the other branch will now read car(x))
        setatom(parent);
        pointer grandparent = car(parent);
        car(parent) = x;
        x = cdr(parent);
    ...
    switch(state) {
        ...
        case st_oct2:
            ...
            else
            {
                ...
                if (state == st_oct1) state=st_oct2;
                else {
                    *p++=c1;
                    state=st_ok;
                }
...
// As a special case, we don't expect `term` to be increm'd going in.
static pointer reverse_in_place(scheme *sc, pointer term, pointer list) {
     term->references++;
...
// `b` is not increm'd (because of how it's used in practice)
static pointer revappend(scheme *sc, pointer a, pointer b) {
    pointer result = a;
    pointer p = b;
    ...
    while (is_pair(p)) {
        pointer item = car(p);
        item->references++;
        result = cons(sc, item, result);
        p = cdr(p);
    }

    // `p` isn't necessarily valid by this point, but `sc->NIL` is!
    if (p == sc->NIL) {
        return result;
    }
...
// Doesn't expect args to be increm'd
int eqv(pointer a, pointer b) {
     if (is_string(a)) {
          ...
          if (is_proc(b))
               ...
               return (0);
     } else if (type(a) == T_C_PTR) {
          return type(b)==T_C_PTR && strlength(a)==strlength(b) && strvalue(a)==strvalue(b);
     } else {
          return (a == b);
     }
}
...
static void new_frame_in_env(scheme *sc, pointer old_env)
{
  ...
  decrem(sc, sc->envir);
  sc->envir = cons(sc, new_frame, old_env);
...
{
  pointer slot = cons(sc, variable, value);
...
static INLINE void new_frame_in_env(scheme *sc, pointer old_env)
{
  sc->NIL->references++;
  decrem(sc, sc->envir);
  sc->envir = cons(sc, sc->NIL, old_env);
  setenvironment(sc->envir);
}
...
// `slot` isn't increm'd, because slots never are.
...
#if USE_ERROR_HOOK
#error USE_ERROR_HOOK doesn''t support reference counting yet
...
#define s_retbool(tf)  BEGIN \
    pointer ret = (tf) ? sc->T : sc->F; \
    ret->references++; \
    s_return(sc, ret); \
END
...
static pointer opexe_0(scheme *sc, enum scheme_opcodes op) {
     ...
     case OP_T0LVL: /* top level */
       ...
       if(file_interactive(sc))
       {
         // j/k user better not be dumb, not messing up my ref counting for their sake!
         //sc->envir = sc->global_env;
         //dump_stack_reset(sc); // This would need to be updated to be ref-counting-aware
         putstr(sc,"\n");
         putstr(sc,prompt);
       }
     ...
     case OP_E1ARGS:     /* eval arguments */
          ...
          if (is_pair(sc->code)) { /* continue */
               x = sc->code;
               cdr(x)->references++;
               s_save(sc,OP_E1ARGS, sc->args, cdr(x));
               sc->code = car(sc->code);
               sc->code->references++;
               sc->NIL->references++;
               sc->args = sc->NIL;
     ...
     case OP_REAL_APPLY:
          ...
          if (is_proc(sc->code)) {
               ...
               // Assume sc->args doesn't need to be increm'd since usually it's just being read;
               // Conversely, have to assume the result is appropriately increm'd, since otherwise
               // it'll be immediately GC'd if it's actually new.
        ...
        /* Should not accept promise */
               /* make environment */
               closure_env(sc->code)->references++;
     ...
     case OP_DEF0:  /* define */
          ...
          if (is_pair(car(sc->code))) {
               // handles (define (f x ...) ...) syntax
               x = caar(sc->code);
               sc->LAMBDA->references++;
               cdar(sc->code)->references++;
               cdr(sc->code)->references++;
               sc->code = cons(sc, sc->LAMBDA, cons(sc, cdar(sc->code), cdr(sc->code)));
          } else {
               x = car(sc->code);
               cadr(sc->code)->references++;
     ...
     case OP_DEF1:  /* define */
          ...
          if (x != sc->NIL) {
               car(x)->references++;
               Error_1(sc, "May not redefine symbol", car(x));
     ...
     case OP_LET0:       /* let */
          ...
          sc->args = sc->NIL;
          sc->value = sc->code;
          //sc->code = is_symbol(car(sc->code)) ? cadr(sc->code) : car(sc->code); // Accomodations for named let, also known as the enemy
          sc->code = car(sc->code);
          sc->code->references++;
          s_goto(sc,OP_LET1);
     ...
     case OP_LET1:       /* let (calculate parameters) */
          ...
          if (is_pair(sc->code)) { /* continue */
               ...
               x = sc->code;
               cdr(x)->references++;
               s_save(sc,OP_LET1, sc->args, cdr(x));
               sc->args = sc->NIL;
               sc->NIL->references++;
               sc->code = cadar(x);
               sc->code->references++;
     ...
     case OP_LET2:       /* let */
          ...
          // Named let accomodations removed here as well
          ...
          /* Named let is illegal because it's another route to recursion >:(
     ...
     case OP_LET0AST:    /* let* */
          ...
          car(x)->references++;
          cdr(x)->references++;
          s_save(sc,OP_LET1AST, cdr(x), car(x));
          sc->code = cadaar(x);
          sc->code->references++;
          decrem(sc, x);
          s_goto(sc,OP_EVAL);
     ...
     case OP_LET1AST:    /* let* (make new frame) */
          ...
          x = sc->code;
          caar(x)->references++;
          new_slot_in_env(sc, caar(x), sc->value);
          sc->code = cdr(sc->code);
          sc->code->references++;
          decrem(sc, x);
          if (is_pair(sc->code)) { /* continue */
...
static pointer opexe_1(scheme *sc, enum scheme_opcodes op) {
     ...
     */
     ...
     case OP_COND1:      /* cond */
          if (is_true(sc->value)) {
               ...
               if(car(sc->code)==sc->FEED_TO) {
                    ...
                    sc->code=cons(sc,cadr(y),cons(sc,x,sc->NIL));
                    decrem(sc, y);
                    s_goto(sc,OP_EVAL);
               }
               s_goto(sc,OP_BEGIN);
     ...
     case OP_AND0:       /* and */
          ...
          // Fall thru
     case OP_AND1:       /* and */
          if (is_false(sc->value) || sc->code == sc->NIL) {
               sc->value->references++;
               s_return(sc,sc->value);
          }
          x = sc->code;
          sc->NIL->references++;
          cdr(x)->references++;
          s_save(sc,OP_AND1, sc->NIL, cdr(x));
          sc->code = car(x);
          sc->code->references++;
          decrem(sc, x);
          s_goto(sc,OP_EVAL);
     ...
     case OP_OR0:        /* or */
          ...
          // Fall thru
     case OP_OR1:        /* or */
          if (is_true(sc->value) || sc->code == sc->NIL) {
               sc->value->references++;
               s_return(sc,sc->value);
          }
          x = sc->code;
     ...
     case OP_C0STREAM:   /* cons-stream */
          ...
          sc->NIL->references++;
          cdr(x)->references++;
          s_save(sc,OP_C1STREAM, sc->NIL, cdr(x));
          sc->code = car(x);
          sc->code->references++;
          decrem(sc, x);
          s_goto(sc,OP_EVAL);
     ...
     case OP_MACRO0:     /* macro */
          ...
          if (is_pair(car(sc->code))) {
               ...
               cdar(y)->references++;
               cdr(y)->references++;
               sc->code = cons(sc, sc->LAMBDA, cons(sc, cdar(y), cdr(y)));
          } else {
               x = car(sc->code);
               cadr(y)->references++;
               sc->code = cadr(y);
     ...
     case OP_MACRO1:     /* macro */
          ...
          x = find_slot_in_env(sc, sc->envir, sc->code, 0);
          // TODO This is danerous if it isn't the root envir
          if (x != sc->NIL) {
               Error_0(sc, "Cannot reassign symbols");
          }
          sc->code->references += 2;
          sc->value->references++;
          new_slot_in_env(sc, sc->code, sc->value);
     ...
     case OP_CASE0:      /* case */
          ...
          sc->NIL->references++;
          cdr(x)->references++;
          s_save(sc,OP_CASE1, sc->NIL, cdr(x));
          sc->code = car(x);
          sc->code->references++;
          decrem(sc, x);
          s_goto(sc,OP_EVAL);
...
static pointer opexe_2(scheme *sc, enum scheme_opcodes op) {
     ...
     case OP_TRUNCATE:
          s_return(sc, mk_integer(sc, (long)rvalue(car(sc->args))));
     ...
     case OP_CONS:       /* cons */
          x = cdr(sc->args);
          car(x)->references++;
          // Apparently we like to live dangerously, and apparently that means editing sc->args in-place.
          cdr(sc->args) = car(x);
          decrem(sc, x);
          sc->args->references++;
          s_return(sc,sc->args);
...
*/
     ...
     case OP_STRREF: { /* string-ref */
          ...
          if(index>=(unsigned int)strlength(car(sc->args))) {
               cadr(sc->args)->references++;
...
*/
     ...
     case OP_SUBSTR: { /* substring */
          ...
          if((unsigned int)index0>(unsigned int)strlength(car(sc->args))) {
               cadr(sc->args)->references++;
     ...
     case OP_VECREF: { /* vector-ref */
          ...
          if((unsigned int)index >= (unsigned int)ivalue(car(sc->args))) {
               cadr(sc->args)->references++;
               Error_1(sc,"vector-ref: out of bounds:",cadr(sc->args));
          }

          x = vector_elem(car(sc->args),index);
          x->references++;
          s_return(sc,x);
     }
...
static pointer opexe_4(scheme *sc, enum scheme_opcodes op) {
     ...
     case OP_SAVE_FORCED:     /* Save forced value replacing promise */
          Error_0(sc, "OP_SAVE_FORCED (promise evaluation) does some weird memory manipulation stuff that hasn't been ported to ref-counting yet");
     ...
     case OP_ERR1:  /* error */
          ...
          if (sc->args != sc->NIL) {
               ...
               cdr(x)->references++;
               sc->NIL->references++;
               s_save(sc,OP_ERR1, cdr(x), sc->NIL);
               sc->args = car(x);
               sc->args->references++;
               decrem(sc, x);
               sc->print_flag = 1;
     ...
     /* TODO-opcleanup (Though maybe we'll want it later?)
     ...
     */
     ...
     case OP_APPEND:    /* append */
          ...
          // Process all but the last item in the list
          while (cdr(y) != sc->NIL) {
...
#if USE_PLIST
// No ref counting here, not sure what these ops are supposed to do but I don't like them
#error "PLIST doesn't work, go away"
     case OP_PUT:        /* put */
          ...
          if (x != sc->NIL)
               cdar(x) = caddr(sc->args); // TODO This is weird mutation sh*t, what's OP_PUT anyway?
          else
     ...
     case OP_NEWSEGMENT: /* new-segment */
          Error_0(sc, "OP_NEWSEGMENT makes no fucking sense");
...
static pointer opexe_5(scheme *sc, enum scheme_opcodes op) {
     ...
     case OP_READ:
          ...
          if(!is_inport(car(sc->args))) {
               x = car(sc->args);
               x->references++;
               Error_1(sc, "read: not an input port:", x);
          }
     ...
     case OP_RDSEXPR:
          ...
          case TOK_SHARP: {
               ...
               if(f==sc->NIL) {
                    ...
                    x = slot_value_in_env(f);
                    sc->NIL->references++;
                    decrem(sc, sc->code);
                    sc->code=cons(sc, x, sc->NIL);
                    s_goto(sc,OP_EVAL);
               }
          }
     ...
     case OP_P0LIST:
          if(is_vector(sc->args)) {
               ...
               cdr(x)->references++;
               sc->NIL->references++;
               s_save(sc,OP_P1LIST, cdr(x), sc->NIL);
               sc->args = car(x);
               decrem(sc, x);
               s_goto(sc,OP_P0LIST);
          }
...
static pointer opexe_6(scheme *sc, enum scheme_opcodes op) {
     ...
     case OP_GET_CLOSURE:     /* get-closure-code */   /* a.k */
          ...
          if (sc->args == sc->NIL) {
               sc->F->references++;
               s_return(sc,sc->F);
          } else if (is_closure(sc->args) || is_macro(sc->args)) {
               sc->LAMBDA->references++;
               x = closure_code(sc->value);
               x->references++;
               s_return(sc,cons(sc, sc->LAMBDA, x));
          } else {
               sc->F->references++;
               s_return(sc,sc->F);
          }
...
// `p` not increm'd, by rationale of this being a descriptive method
static int syntaxnum(pointer p) {
...
scheme *scheme_init_new() {
  ...
  // This isn't even supposed to be eval'd - but it is.
  sc->_EOF_OBJ._flag = 0;
  if(!scheme_init(sc)) {
    free(sc);
    return 0;
...
int scheme_init_custom_alloc(scheme *sc, func_alloc malloc, func_dealloc free) {
  ...
  car(sc->FREE_CELL) = cdr(sc->FREE_CELL) = sc->FREE_CELL;
  ...
  /* init sc->NIL */
  typeflag(sc->NIL) = T_ATOM;
  car(sc->NIL) = cdr(sc->NIL) = sc->NIL;
  sc->NIL->references = 11;
  // NIL, inport, outport, save_inport, loadport, c_nest, dummy envir, cdr of first real envir
  // code, args, value
  // We don't count the car/cdr of NIL itself (or `sink`) because
  // those cells are never going to be reclaimed anyway, so why bother counting.

  /* init T */
  typeflag(sc->T) = T_ATOM;
  car(sc->T) = cdr(sc->T) = sc->T;
  sc->T->references = 1;
  /* init F */
  typeflag(sc->F) = T_ATOM;
  car(sc->F) = cdr(sc->F) = sc->F;
  sc->F->references = 1;
  /* init F */
  typeflag(sc->EOF_OBJ) = T_ATOM;
  sc->EOF_OBJ->references = 1;
  /* init sink */
  // Originally we just set `car(sc->sink)` and left the `cdr` alone - not sure why, or if this is ever needed,
  // but I feel like this is a safe place to leave it.
  typeflag(sc->sink) = T_PAIR;
  car(sc->sink) = sc->NIL;
  cdr(sc->sink) = sc->NIL;
  sc->sink->references = 1;
  /* init c_nest */
  sc->c_nest = sc->NIL;
...
#define CLEANUP(x) decrem(sc, x); x=sc->NIL
  ...
  // TODO: Do we need to make sure all memory is released here? Can we expect it to all be released?

  // TODO: Make sure all memory is appropriately freed (e.g. external references from FFI-land)
  for(i=0; i<sc->num_cell_segs; i++) {
...
{
  pointer old_nest = sc->c_nest;

  pointer saved_data = car(old_nest);
  decrem(sc, sc->envir);
  sc->envir = car(saved_data);
  sc->envir->references++;
  sc->dump_zero = ivalue(cdr(saved_data));

  /* Pop */
  sc->c_nest = cdr(old_nest);
  sc->c_nest->references++;
  decrem(sc, old_nest);
}

/* "func" and "args" are assumed to be already eval'ed. */
// result is not increm'd (on the assumption that it will often be discarded)
...
pointer scheme_eval(scheme *sc, pointer obj)
{
  int old_repl = sc->interactive_repl;
  sc->interactive_repl = 0;
  // This is now handled by whoever calls scheme_eval, to avoid memory leaks associated with argument creation.
  //save_from_C_call(sc);
  decrem(sc, sc->args);
  decrem(sc, sc->code);
  sc->NIL->references++;
